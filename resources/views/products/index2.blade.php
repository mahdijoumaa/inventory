                            ...
                            <tbody>
                                @forelse ($products as $index => $product)
                                    <tr>
                                        ...
                                        <td>{{ $product->supplier?->supp_name ?? '-' }}</td>
                                        <td>{{ $product->brand ?? '-' }}</td>
                                        <td>{{ $product->unit ?? '-' }}</td>
                                        ...
                                        <td>
                                            @php
                                                $gp = $product->cost_price > 0 ? (($product->sell_price - $product->cost_price) / $product->cost_price) * 100 : 0;
                                            @endphp
                                            {{ number_format($gp,2) }}%
                                        </td>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content shadow-lg">
            <div class="modal-header bg-primary text-white py-2">
                <h6 class="modal-title mb-0" id="modalProductName">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        if (document.querySelector('#myTable tbody tr:not(:has(td[colspan]))')) {
            new DataTable('#myTable', {
                ...
                columnDefs: [{ orderable: false, targets: [1, 17] }]
            });
        }
